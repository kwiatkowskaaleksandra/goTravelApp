...
/**
 * The FacebookOAuth2UserInfoExtractor class provides methods for extracting user details from a Facebook OAuth2 user and determining if the extractor can handle a specific OAuth2UserRequest.
 */
@Service
public class FacebookOAuth2UserInfoExtractor implements OAuth2UserInfoExtractor{

    /**
     * Extracts user details from the provided Facebook OAuth2User.
     *
     * @param oAuth2User The Facebook OAuth2User containing user information.
    ...
    public UserDetailsImpl extractUserInfo(OAuth2User oAuth2User) {
        ...
        userDetails.setFirstname(retrieveAttr("name", oAuth2User));
        userDetails.setProvider(OAuth2Provider.FACEBOOK);
    ...
    /**
     * Determines whether the extractor can handle the given OAuth2UserRequest for Facebook.
     *
