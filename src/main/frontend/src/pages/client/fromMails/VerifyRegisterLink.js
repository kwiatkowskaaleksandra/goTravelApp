...
import './VerifyRegisterLink.css'

class VerifyRegisterLink extends Component {

    state = {
        activatingTheAccount: false
    }

    componentDidMount() {
        const urlSearchParams = new URLSearchParams(window.location.search);
        const codeParam = urlSearchParams.get('code');
        goTravelApi.verifyRegisterLink(codeParam).then(() => {
            this.setState({activatingTheAccount: true})
        }).catch(error => {
    ...
    render() {
        const {t} = this.props
        const {activatingTheAccount} = this.state

        return (
            <div>
                <div className="bg-light justify-content-center" style={{height: '1000px'}}>
                    <div className="container-fluid" style={{width: '1200px'}}>
                        <div className="row linkVerify">
                            <div className="col-lg-10 offset-lg-1" style={{width: '900px', height: '950px'}}>
...
export default withTranslation()(VerifyRegisterLink);