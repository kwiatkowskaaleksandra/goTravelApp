...
class ShortTrip extends Component {
    ...
    handleGetTrips = () => {
        orderApi.getTrips('krotkiUrlop').then(res => {
    ...
    handleSubmit = () => {
        orderApi.getTripsByFilters('krotkiUrlop', this.state.selectedCountryId, this.state.selectedTransportId, this.state.priceMin, this.state.priceMax, this.state.numberOfDaysMin, this.state.numberOfDaysMax).then(res => {
            this.setState({trips: res.data})
        })
    }
    ...
    handleClear = () => {
        ...
        window.location.href = "/shortTrips"
    }
