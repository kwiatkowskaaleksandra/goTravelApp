...
class Exotic extends Component {
    ...
    handleGetTrips = () => {
        orderApi.getTrips('egzotyka').then(res => {
    ...
    handleSubmit = () => {
        orderApi.getTripsByFilters('egzotyka', this.state.selectedCountryId, this.state.selectedTransportId, this.state.priceMin, this.state.priceMax, this.state.numberOfDaysMin, this.state.numberOfDaysMax).then(res => {
            this.setState({trips: res.data})
        })
    }
