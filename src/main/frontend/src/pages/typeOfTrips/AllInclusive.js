...
class AllInclusive extends Component {
    ...
    handleGetTrips = () => {
        orderApi.getTrips('allInclusive').then(res => {
    ...
    handleSubmit = () => {
        orderApi.getTripsByFilters('allInclusive', this.state.selectedCountryId, this.state.selectedTransportId, this.state.priceMin, this.state.priceMax, this.state.numberOfDaysMin, this.state.numberOfDaysMax).then(res => {
            this.setState({trips: res.data})
        })
    }
    ...
    handleClear = () => {
        ...
        window.location.href = "/allInclusive"
    }
